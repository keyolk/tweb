...
Electron과 Tauri 모두 열린 tab URL, active tab, tab별 zoom을 tmux window 단위로 저장합니다. `tweb open`처럼 URL을 생략하면 마지막 상태를 복원하고, URL을 명시하면 저장 상태를 무시해 해당 URL 하나로 새로 시작합니다. Electron은 main frame의 load 완료가 확인된 URL만 저장합니다. DNS·connection·file load가 실패한 요청과 `about:blank`, 내부 loading/오류/안내 page는 session을 갱신하지 않으며, 실패 전에 성공한 URL이 있으면 그 상태를 유지합니다. 성공이 확인된 복원 URL이 없으면 검은 화면 대신 주소 입력 안내를 표시합니다. Electron persistence key는 tmux socket, session 이름, window index를 사용하므로 tmux server를 재시작해도 같은 slot을 복원합니다. session 이름이나 window index를 바꾸면 별도 상태로 취급합니다.
